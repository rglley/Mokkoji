...
public class PhotomosaicService {
    ...
    public MultipartFile createPhotomosaic(String thumbnailPath, String cellImagesPath) {
        ...
        try {
            tempFilePath = Files.createTempFile("photomosaic", ".jpeg");
            Imgcodecs.imwrite(tempFilePath.toString(), blankImage);

            byte[] fileContent = new byte[0];
            fileContent = Files.readAllBytes(tempFilePath);
            MultipartFile multipartFile = new MockMultipartFile("file", "photomosaic.jpeg", "image/jpeg", fileContent);
